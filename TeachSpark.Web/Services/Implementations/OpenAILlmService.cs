...
{
    private readonly LlmConfiguration _config = config.Value;
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            ...
            try
            {
                var modelResult = await modelRegistry.GetEducationRecommendedModelsAsync();
            ...
            {
                logger.LogWarning(costEx, "Failed to calculate cost for RequestId: {RequestId}", requestId);
            }
            ...
            logger.LogInformation("Successfully generated worksheet content for RequestId: {RequestId}", requestId);
        ...
        catch (Exception ex)
        {
            ...
            {
                logger.LogError(loggingEx, "Failed to log LLM error for RequestId: {RequestId}", requestId);
            }
    ...
    public async Task<ServiceResult<List<string>>> GetAvailableModelsAsync(CancellationToken cancellationToken = default)
    {
        try
        {
            var modelsResult = await modelRegistry.GetEducationRecommendedModelsAsync(cancellationToken);
        ...
        {
            logger.LogError(ex, "Failed to get available models");
            return ServiceResult<List<string>>.ErrorResult("Failed to retrieve available models");
        }
    }
    ...
    public async Task<ServiceResult<bool>> ValidateConfigurationAsync(CancellationToken cancellationToken = default)
    {
        try
        {
            ...
            var response = await chatClient.CompleteChatAsync(testMessages, testOptions, cancellationToken);
    ...
    private void UpdateMetrics(bool success, decimal cost, int tokens, TimeSpan duration)
    {
        ...
        metrics.TotalProcessingTime = metrics.TotalProcessingTime.Add(duration);
    ...
    private async Task<string> GeneratePromptAsync(WorksheetGenerationRequest request, string worksheetType)
    {
        ...
        if (request.CommonCoreStandardId.HasValue)
        {
            ...
            {
                logger.LogWarning(ex, "Failed to fetch Common Core Standard with ID: {StandardId}", request.CommonCoreStandardId);
            }
        }
        ...
        if (request.BloomLevelId.HasValue)
        {
            ...
            {
                logger.LogWarning(ex, "Failed to fetch Bloom Level with ID: {BloomLevelId}", request.BloomLevelId);
            }
        }
...
Maintain a professional, educational tone while keeping content engaging for students.";
        ...
        TimeSpan generationTime)
    {
        ...
        {
            logger.LogWarning(ex, "Failed to calculate accurate cost, using estimate");
        }
    ...
    private string RenderMarkdownToHtml(string markdownContent)
    {
        try
        {
            return markdownService.RenderToHtml(markdownContent);
        }
        catch (Exception ex)
        {
            logger.LogWarning(ex, "Failed to render markdown to HTML, returning original content");
